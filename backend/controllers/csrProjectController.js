...
const getImpactUpdates = async (req, res) => {
  try {
    const { id } = req.params;
    const updates = await ImpactUpdate.find({ projectId: id }).sort({ createdAt: -1 });
    return res.status(200).json({ updates });
  } catch (error) {
    return res.status(500).json({ message: "Unable to fetch impact updates" });
  }
};

const addImpactUpdate = async (req, res) => {
  try {
    const { id } = req.params;
    const { metricName, metricValue, metricUnit, description } = req.body;

    const newUpdate = await ImpactUpdate.create({
      projectId: id,
      metricName,
      metricValue: Number(metricValue),
      metricUnit,
      description,
      category: 'outcome', // default
      verificationStatus: 'pending' // requires admin/corporate approval
    });

    return res.status(201).json({ update: newUpdate, message: "Impact update added successfully" });
  } catch (error) {

...
const matchmakingController = {
  ...
  getMatchReasons: (ngo, project) => {
    ...
    const ngoFocus = ngo.focusAreas || [];
    const projectFocus = project.category || '';
    const commonFocus = ngoFocus.filter(f => 
  ...
  getNgoMatches: async (req, res) => {
    try {
      ...
      let projectQuery = { visibility: 'public', approvalStatus: 'approved', status: 'active' };
      if (focusArea) {
        projectQuery.category = focusArea;
      }
      if (location) {
  ...
  getCorporateMatches: async (req, res) => {
    try {
      ...
      let projectQuery = { corporate: corporateId, visibility: 'public', approvalStatus: 'approved', status: 'active' };
      const projects = await CSRProject.find(projectQuery);
  ...
  getMatchDetails: async (req, res) => {
    try {
      ...
      const ngo = await NGO.findById(ngoId);
      const project = await CSRProject.findById(projectId).populate('corporate', 'companyName');

...
const getCorporateStats = async (req, res) => {
  try {
    ...
    const [projects, partnerships] = await Promise.all([
      CSRProject.find({ corporate: corporate._id }),
      Partnership.find({ corporateId: corporate._id }),
    ]);

    const totalProjects = projects.length;
    const approvedProjects = projects.filter((project) => project.approvalStatus === "approved").length;
    const pendingProjects = projects.filter((project) => project.approvalStatus === "pending").length;
    const rejectedProjects = projects.filter((project) => project.approvalStatus === "rejected").length;
    const totalCommitted = projects.reduce((sum, project) => sum + (project.budget || 0), 0);
    const totalDisbursed = partnerships.reduce(
      (sum, partnership) => sum + (partnership.allocatedBudget || 0),
      0
    );
    const activePartnerships = partnerships.filter(
      (partnership) => partnership.status === "active" || partnership.status === "accepted"
    ).length;
    ...
    return res.status(500).json({ message: "Unable to load corporate stats" });
  }
};
